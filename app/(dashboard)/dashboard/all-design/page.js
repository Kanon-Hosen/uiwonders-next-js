        ...
        <div className="mt-6 ">
          {data.map((design) => {
            return <div className="w-full flex gap-5">
              <Image src={design?.images[0]} alt={design?.name} width={100} height={100} className="rounded-md" />
              <div className="flex flex-col gap-2 text-black">
              <h1 className="font-semibold text-lg">{design?.name}</h1>
              <p className="italic text-sm text-neutral-700">Design by - {design?.authorName}</p>
              </div>
              <div className="flex flex-col gap-2 ml-4 ">
                <Link className="bg-green-500 rounded px-3 py-1 text-white text-sm" href='/'>Edit</Link>
                <button className="bg-red-500 rounded px-3 py-1 text-white text-sm">Delete</button>
              </div>
            </div>;
          })}
        </div>
