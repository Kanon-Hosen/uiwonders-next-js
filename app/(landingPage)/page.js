...
export default function Home() {
  ...
  return (
    <main>
      ...
      <section className="md:px-8 px-6 z-20 mt-10 h-fit">
        <h1 className="capitalize text-4xl z-40 text-black text-center font-bold ">
          Trending Designs
        </h1>
        <p className="md:text-lg text-center mt-2">
          Explore Our Tranding Design And Download Free
        </p>
        ...
        {data && (
          <div className="grid md:grid-cols-3 gap-3 mt-8">
            {data &&
              data.map((design) => <DesignCard data={design}></DesignCard>)}
          </div>
        )}
        {!data && (
          <div className="flex items-center h-96 justify-center my-12 text-xl font-bold text-black">
            <p>No Design Found</p>
          </div>
        )}
      </section>
